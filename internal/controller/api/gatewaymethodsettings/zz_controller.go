...
// Setup adds a controller that reconciles GatewayMethodSettings managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.GatewayMethodSettingsGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.GatewayMethodSettingsGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_api_gateway_method_settings"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.GatewayMethodSettings{}).
		Complete(r)
}
