...
// Setup adds a controller that reconciles DbProxyDefaultTargetGroup managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.DbProxyDefaultTargetGroupGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.DbProxyDefaultTargetGroupGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_db_proxy_default_target_group"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.DbProxyDefaultTargetGroup{}).
		Complete(r)
}
