...
// Setup adds a controller that reconciles DbEventSubscription managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.DbEventSubscriptionGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.DbEventSubscriptionGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_db_event_subscription"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.DbEventSubscription{}).
		Complete(r)
}
