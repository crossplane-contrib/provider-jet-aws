...
// Setup adds a controller that reconciles DbClusterSnapshot managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.DbClusterSnapshotGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.DbClusterSnapshotGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_db_cluster_snapshot"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.DbClusterSnapshot{}).
		Complete(r)
}
