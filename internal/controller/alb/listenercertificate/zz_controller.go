...
import (
	...
	v1alpha1 "github.com/crossplane-contrib/provider-jet-aws/apis/alb/v1alpha1"
)

// Setup adds a controller that reconciles ListenerCertificate managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.ListenerCertificate_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.ListenerCertificate_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_alb_listener_certificate"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.ListenerCertificate{}).
		Complete(r)
}
