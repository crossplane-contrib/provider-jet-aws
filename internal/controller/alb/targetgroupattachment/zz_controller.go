...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.TargetGroupAttachmentGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.TargetGroupAttachmentGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_alb_target_group_attachment"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.TargetGroupAttachment{}).
		Complete(r)
}
