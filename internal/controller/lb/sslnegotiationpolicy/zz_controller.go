...
// Setup adds a controller that reconciles SslNegotiationPolicy managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.SslNegotiationPolicyGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.SslNegotiationPolicyGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_lb_ssl_negotiation_policy"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.SslNegotiationPolicy{}).
		Complete(r)
}
