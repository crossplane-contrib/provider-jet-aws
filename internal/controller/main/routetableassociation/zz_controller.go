...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.RouteTableAssociationGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.RouteTableAssociationGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_main_route_table_association"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.RouteTableAssociation{}).
		Complete(r)
}
