...
import (
	...
	v1alpha1 "github.com/crossplane-contrib/provider-jet-aws/apis/cloudhsm/v1alpha1"
)

// Setup adds a controller that reconciles V2Cluster managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.V2Cluster_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.V2Cluster_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_cloudhsm_v2_cluster"])),
