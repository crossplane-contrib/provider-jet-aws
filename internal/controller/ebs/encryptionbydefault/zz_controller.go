...
import (
	...
	v1alpha1 "github.com/crossplane-contrib/provider-jet-aws/apis/ebs/v1alpha1"
)

// Setup adds a controller that reconciles EncryptionByDefault managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.EncryptionByDefault_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.EncryptionByDefault_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_ebs_encryption_by_default"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.EncryptionByDefault{}).
		Complete(r)
}
