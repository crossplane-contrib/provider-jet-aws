...
// Setup adds a controller that reconciles Route53ResolverFirewallDomainList managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.Route53ResolverFirewallDomainListGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.Route53ResolverFirewallDomainListGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_route53_resolver_firewall_domain_list"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.Route53ResolverFirewallDomainList{}).
		Complete(r)
}
