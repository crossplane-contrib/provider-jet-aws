...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.MainRouteTableAssociation_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.MainRouteTableAssociation_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_main_route_table_association"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.MainRouteTableAssociation{}).
		Complete(r)
}
