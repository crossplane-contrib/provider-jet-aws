...
// Setup adds a controller that reconciles NetworkfirewallFirewall managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.NetworkfirewallFirewallGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.NetworkfirewallFirewallGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_networkfirewall_firewall"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.NetworkfirewallFirewall{}).
		Complete(r)
}
