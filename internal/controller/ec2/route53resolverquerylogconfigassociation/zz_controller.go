...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.Route53ResolverQueryLogConfigAssociationGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.Route53ResolverQueryLogConfigAssociationGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_route53_resolver_query_log_config_association"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.Route53ResolverQueryLogConfigAssociation{}).
		Complete(r)
}
