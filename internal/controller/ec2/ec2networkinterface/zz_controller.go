...
// Setup adds a controller that reconciles EC2NetworkInterface managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.EC2NetworkInterfaceGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.EC2NetworkInterfaceGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_network_interface"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.EC2NetworkInterface{}).
		Complete(r)
}
