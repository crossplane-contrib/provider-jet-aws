...
// Setup adds a controller that reconciles Route53HostedZoneDnssec managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.Route53HostedZoneDnssecGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.Route53HostedZoneDnssecGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_route53_hosted_zone_dnssec"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.Route53HostedZoneDnssec{}).
		Complete(r)
}
