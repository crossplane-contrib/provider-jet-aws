...
// Setup adds a controller that reconciles NetworkfirewallResourcePolicy managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.NetworkfirewallResourcePolicyGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.NetworkfirewallResourcePolicyGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_networkfirewall_resource_policy"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.NetworkfirewallResourcePolicy{}).
		Complete(r)
}
