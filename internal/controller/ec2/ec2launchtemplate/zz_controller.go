...
// Setup adds a controller that reconciles EC2LaunchTemplate managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.EC2LaunchTemplateGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.EC2LaunchTemplateGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_launch_template"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.EC2LaunchTemplate{}).
		Complete(r)
}
