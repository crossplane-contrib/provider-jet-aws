...
// Setup adds a controller that reconciles NetworkfirewallFirewallPolicy managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.NetworkfirewallFirewallPolicyGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.NetworkfirewallFirewallPolicyGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_networkfirewall_firewall_policy"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.NetworkfirewallFirewallPolicy{}).
		Complete(r)
}
