...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.NetworkfirewallLoggingConfigurationGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.NetworkfirewallLoggingConfigurationGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_networkfirewall_logging_configuration"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.NetworkfirewallLoggingConfiguration{}).
		Complete(r)
}
