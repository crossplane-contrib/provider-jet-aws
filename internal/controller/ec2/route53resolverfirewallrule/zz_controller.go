...
// Setup adds a controller that reconciles Route53ResolverFirewallRule managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.Route53ResolverFirewallRuleGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.Route53ResolverFirewallRuleGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_route53_resolver_firewall_rule"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.Route53ResolverFirewallRule{}).
		Complete(r)
}
