...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.TransitGatewayRouteTableGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.TransitGatewayRouteTableGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_ec2_transit_gateway_route_table"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.TransitGatewayRouteTable{}).
		Complete(r)
}
