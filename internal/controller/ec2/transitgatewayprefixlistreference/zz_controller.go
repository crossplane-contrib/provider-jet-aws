...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.TransitGatewayPrefixListReferenceGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.TransitGatewayPrefixListReferenceGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_ec2_transit_gateway_prefix_list_reference"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.TransitGatewayPrefixListReference{}).
		Complete(r)
}
