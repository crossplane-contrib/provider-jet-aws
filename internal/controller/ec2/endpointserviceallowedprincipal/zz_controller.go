...
package endpointserviceallowedprincipal
...
// Setup adds a controller that reconciles EndpointServiceAllowedPrincipal managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.EndpointServiceAllowedPrincipal_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.EndpointServiceAllowedPrincipal_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_vpc_endpoint_service_allowed_principal"])),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.EndpointServiceAllowedPrincipal{}).
		Complete(r)
}
