...
// Setup adds a controller that reconciles BeanstalkApplication managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, concurrency int) error {
	name := managed.ControllerName(v1alpha1.BeanstalkApplicationGroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.BeanstalkApplicationGroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, config.Store.GetForResource("aws_elastic_beanstalk_application"))),
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&v1alpha1.BeanstalkApplication{}).
		Complete(r)
}
