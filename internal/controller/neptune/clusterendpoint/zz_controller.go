...
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha2.ClusterEndpoint_GroupVersionKind.String())
	var initializers managed.InitializerChain
	for _, i := range cfg.Resources["aws_neptune_cluster_endpoint"].InitializerFns {
		initializers = append(initializers, i(mgr.GetClient()))
	}
	initializers = append(initializers, managed.NewNameAsExternalName(mgr.GetClient()))
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha2.ClusterEndpoint_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["aws_neptune_cluster_endpoint"])),
