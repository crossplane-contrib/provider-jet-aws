/*
Copyright 2021 The Crossplane Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by terrajet. DO NOT EDIT.

// +kubebuilder:object:generate=true
// +groupName=iot.aws.tf.crossplane.io
// +versionName=v1alpha1

package v1alpha1

import (
	xpv1 "github.com/crossplane/crossplane-runtime/apis/common/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"

	apis "github.com/crossplane-contrib/provider-tf-aws/apis"
	v1alpha1 "github.com/crossplane-contrib/provider-tf-aws/apis/iot/v1alpha1"
)

type IotCertificateObservation struct {
	Arn string `json:"arn" tf:"arn"`

	CertificatePem string `json:"certificatePem" tf:"certificate_pem"`

	PrivateKey string `json:"privateKey" tf:"private_key"`

	PublicKey string `json:"publicKey" tf:"public_key"`
}

type IotCertificateParameters struct {
	Active bool `json:"active" tf:"active"`

	Csr *string `json:"csr,omitempty" tf:"csr"`
}

// IotCertificateSpec defines the desired state of IotCertificate
type IotCertificateSpec struct {
	xpv1.ResourceSpec `json:",inline"`
	ForProvider       IotCertificateParameters `json:"forProvider"`
}

// IotCertificateStatus defines the observed state of IotCertificate.
type IotCertificateStatus struct {
	xpv1.ResourceStatus `json:",inline"`
	AtProvider          IotCertificateObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true

// IotCertificate is the Schema for the IotCertificates API
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:subresource:status
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,aws}
type IotCertificate struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	Spec              IotCertificateSpec   `json:"spec"`
	Status            IotCertificateStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// IotCertificateList contains a list of IotCertificates
type IotCertificateList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []IotCertificate `json:"items"`
}

// Repository type metadata.
var (
	IotCertificateKind             = "IotCertificate"
	IotCertificateGroupKind        = schema.GroupKind{Group: v1alpha1.Group, Kind: IotCertificateKind}.String()
	IotCertificateKindAPIVersion   = IotCertificateKind + "." + v1alpha1.GroupVersion.String()
	IotCertificateGroupVersionKind = v1alpha1.GroupVersion.WithKind(IotCertificateKind)
)

func init() {
	apis.SchemaMap[v1alpha1.GroupVersion] = append(apis.SchemaMap[v1alpha1.GroupVersion], &IotCertificate{}, &IotCertificateList{})
}
