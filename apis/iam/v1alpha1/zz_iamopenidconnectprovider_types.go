/*
Copyright 2021 The Crossplane Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by terrajet. DO NOT EDIT.

package v1alpha1

import (
	xpv1 "github.com/crossplane/crossplane-runtime/apis/common/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"
)

type IamOpenidConnectProviderObservation struct {
	Arn string `json:"arn" tf:"arn"`
}

type IamOpenidConnectProviderParameters struct {
	ClientIdList []string `json:"clientIdList" tf:"client_id_list"`

	Tags map[string]string `json:"tags,omitempty" tf:"tags"`

	TagsAll map[string]string `json:"tagsAll,omitempty" tf:"tags_all"`

	ThumbprintList []string `json:"thumbprintList" tf:"thumbprint_list"`

	Url string `json:"url" tf:"url"`
}

// IamOpenidConnectProviderSpec defines the desired state of IamOpenidConnectProvider
type IamOpenidConnectProviderSpec struct {
	xpv1.ResourceSpec `json:",inline"`
	ForProvider       IamOpenidConnectProviderParameters `json:"forProvider"`
}

// IamOpenidConnectProviderStatus defines the observed state of IamOpenidConnectProvider.
type IamOpenidConnectProviderStatus struct {
	xpv1.ResourceStatus `json:",inline"`
	AtProvider          IamOpenidConnectProviderObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true

// IamOpenidConnectProvider is the Schema for the IamOpenidConnectProviders API
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:subresource:status
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,tfaws}
type IamOpenidConnectProvider struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	Spec              IamOpenidConnectProviderSpec   `json:"spec"`
	Status            IamOpenidConnectProviderStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// IamOpenidConnectProviderList contains a list of IamOpenidConnectProviders
type IamOpenidConnectProviderList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []IamOpenidConnectProvider `json:"items"`
}

// Repository type metadata.
var (
	IamOpenidConnectProviderKind             = "IamOpenidConnectProvider"
	IamOpenidConnectProviderGroupKind        = schema.GroupKind{Group: Group, Kind: IamOpenidConnectProviderKind}.String()
	IamOpenidConnectProviderKindAPIVersion   = IamOpenidConnectProviderKind + "." + GroupVersion.String()
	IamOpenidConnectProviderGroupVersionKind = GroupVersion.WithKind(IamOpenidConnectProviderKind)
)

func init() {
	SchemeBuilder.Register(&IamOpenidConnectProvider{}, &IamOpenidConnectProviderList{})
}
