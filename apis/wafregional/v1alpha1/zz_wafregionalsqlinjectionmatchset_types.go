/*
Copyright 2021 The Crossplane Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by terrajet. DO NOT EDIT.

package v1alpha1

import (
	xpv1 "github.com/crossplane/crossplane-runtime/apis/common/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"
)

type SqlInjectionMatchTupleFieldToMatchObservation struct {
}

type SqlInjectionMatchTupleFieldToMatchParameters struct {
	Data *string `json:"data,omitempty" tf:"data"`

	Type string `json:"type" tf:"type"`
}

type SqlInjectionMatchTupleObservation struct {
}

type SqlInjectionMatchTupleParameters struct {
	FieldToMatch []SqlInjectionMatchTupleFieldToMatchParameters `json:"fieldToMatch" tf:"field_to_match"`

	TextTransformation string `json:"textTransformation" tf:"text_transformation"`
}

type WafregionalSqlInjectionMatchSetObservation struct {
}

type WafregionalSqlInjectionMatchSetParameters struct {
	Name string `json:"name" tf:"name"`

	Region string `json:"region" tf:"-"`

	SqlInjectionMatchTuple []SqlInjectionMatchTupleParameters `json:"sqlInjectionMatchTuple,omitempty" tf:"sql_injection_match_tuple"`
}

// WafregionalSqlInjectionMatchSetSpec defines the desired state of WafregionalSqlInjectionMatchSet
type WafregionalSqlInjectionMatchSetSpec struct {
	xpv1.ResourceSpec `json:",inline"`
	ForProvider       WafregionalSqlInjectionMatchSetParameters `json:"forProvider"`
}

// WafregionalSqlInjectionMatchSetStatus defines the observed state of WafregionalSqlInjectionMatchSet.
type WafregionalSqlInjectionMatchSetStatus struct {
	xpv1.ResourceStatus `json:",inline"`
	AtProvider          WafregionalSqlInjectionMatchSetObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true

// WafregionalSqlInjectionMatchSet is the Schema for the WafregionalSqlInjectionMatchSets API
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:subresource:status
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,tfaws}
type WafregionalSqlInjectionMatchSet struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	Spec              WafregionalSqlInjectionMatchSetSpec   `json:"spec"`
	Status            WafregionalSqlInjectionMatchSetStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// WafregionalSqlInjectionMatchSetList contains a list of WafregionalSqlInjectionMatchSets
type WafregionalSqlInjectionMatchSetList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []WafregionalSqlInjectionMatchSet `json:"items"`
}

// Repository type metadata.
var (
	WafregionalSqlInjectionMatchSetKind             = "WafregionalSqlInjectionMatchSet"
	WafregionalSqlInjectionMatchSetGroupKind        = schema.GroupKind{Group: Group, Kind: WafregionalSqlInjectionMatchSetKind}.String()
	WafregionalSqlInjectionMatchSetKindAPIVersion   = WafregionalSqlInjectionMatchSetKind + "." + GroupVersion.String()
	WafregionalSqlInjectionMatchSetGroupVersionKind = GroupVersion.WithKind(WafregionalSqlInjectionMatchSetKind)
)

func init() {
	SchemeBuilder.Register(&WafregionalSqlInjectionMatchSet{}, &WafregionalSqlInjectionMatchSetList{})
}
