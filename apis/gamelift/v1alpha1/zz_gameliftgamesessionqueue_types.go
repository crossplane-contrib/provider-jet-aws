/*
Copyright 2021 The Crossplane Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by terrajet. DO NOT EDIT.

package v1alpha1

import (
	xpv1 "github.com/crossplane/crossplane-runtime/apis/common/v1"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime/schema"
)

type GameliftGameSessionQueueObservation struct {
	ARN string `json:"arn" tf:"arn"`
}

type GameliftGameSessionQueueParameters struct {
	Destinations []string `json:"destinations,omitempty" tf:"destinations"`

	Name string `json:"name" tf:"name"`

	PlayerLatencyPolicy []PlayerLatencyPolicyParameters `json:"playerLatencyPolicy,omitempty" tf:"player_latency_policy"`

	Tags map[string]string `json:"tags,omitempty" tf:"tags"`

	TagsAll map[string]string `json:"tagsAll,omitempty" tf:"tags_all"`

	TimeoutInSeconds *int64 `json:"timeoutInSeconds,omitempty" tf:"timeout_in_seconds"`
}

type PlayerLatencyPolicyObservation struct {
}

type PlayerLatencyPolicyParameters struct {
	MaximumIndividualPlayerLatencyMilliseconds int64 `json:"maximumIndividualPlayerLatencyMilliseconds" tf:"maximum_individual_player_latency_milliseconds"`

	PolicyDurationSeconds *int64 `json:"policyDurationSeconds,omitempty" tf:"policy_duration_seconds"`
}

// GameliftGameSessionQueueSpec defines the desired state of GameliftGameSessionQueue
type GameliftGameSessionQueueSpec struct {
	xpv1.ResourceSpec `json:",inline"`
	ForProvider       GameliftGameSessionQueueParameters `json:"forProvider"`
}

// GameliftGameSessionQueueStatus defines the observed state of GameliftGameSessionQueue.
type GameliftGameSessionQueueStatus struct {
	xpv1.ResourceStatus `json:",inline"`
	AtProvider          GameliftGameSessionQueueObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true

// GameliftGameSessionQueue is the Schema for the GameliftGameSessionQueues API
// +kubebuilder:printcolumn:name="READY",type="string",JSONPath=".status.conditions[?(@.type=='Ready')].status"
// +kubebuilder:printcolumn:name="SYNCED",type="string",JSONPath=".status.conditions[?(@.type=='Synced')].status"
// +kubebuilder:printcolumn:name="EXTERNAL-NAME",type="string",JSONPath=".metadata.annotations.crossplane\\.io/external-name"
// +kubebuilder:printcolumn:name="AGE",type="date",JSONPath=".metadata.creationTimestamp"
// +kubebuilder:subresource:status
// +kubebuilder:resource:scope=Cluster,categories={crossplane,managed,tfaws}
type GameliftGameSessionQueue struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`
	Spec              GameliftGameSessionQueueSpec   `json:"spec"`
	Status            GameliftGameSessionQueueStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// GameliftGameSessionQueueList contains a list of GameliftGameSessionQueues
type GameliftGameSessionQueueList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []GameliftGameSessionQueue `json:"items"`
}

// Repository type metadata.
var (
	GameliftGameSessionQueueKind             = "GameliftGameSessionQueue"
	GameliftGameSessionQueueGroupKind        = schema.GroupKind{Group: Group, Kind: GameliftGameSessionQueueKind}.String()
	GameliftGameSessionQueueKindAPIVersion   = GameliftGameSessionQueueKind + "." + GroupVersion.String()
	GameliftGameSessionQueueGroupVersionKind = GroupVersion.WithKind(GameliftGameSessionQueueKind)
)

func init() {
	SchemeBuilder.Register(&GameliftGameSessionQueue{}, &GameliftGameSessionQueueList{})
}
